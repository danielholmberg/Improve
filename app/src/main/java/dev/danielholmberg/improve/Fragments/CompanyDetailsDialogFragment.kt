...
class CompanyDetailsDialogFragment : DialogFragment() {
    ...
    private var companyContacts: HashMap<String, Any>? = null
    ...
    private fun deleteCompany(company: Company?) {
        if (getActivity() != null) {
            ...
            Snackbar.make(
                getActivity()!!.findViewById(R.id.contacts_fragment_container),
                "Deleted company: " + company.name,
                Snackbar.LENGTH_LONG
            )
                .setAction("UNDO") { databaseManager!!.addCompany(company) }
                .show()
        }
        dismissDialog()
    }
